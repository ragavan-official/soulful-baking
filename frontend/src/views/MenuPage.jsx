...
const MenuPage = ({ user, onLogout }) => {
  ...
  const openWhatsApp = (item, selectedFlavour, selectedQty = 1) => {
    const hasFlavours = item.flavours && item.flavours.length > 0;
    const isCake = hasFlavours || item.category?.toLowerCase().includes('cake');
  ...
  const getItemsByCategory = (catName) => {
    return filtered.filter(item => item.category === catName);
  };
          ...
          {loading ? (
            ...
            >
              {categoriesToRender.map(catName => {
                const itemsInCat = getItemsByCategory(catName);
                      ...
                      <table className="menu-list-table" style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '600px' }}>
                        <thead>
                          <tr style={{ borderBottom: '1px solid rgba(229,169,60,0.15)', color: 'var(--text-secondary)', fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                            <th style={{ padding: '0.8rem 1rem' }}>Item Description</th>
                        ...
                        <tbody>
                          {itemsInCat.map(item => {
                            ...
                            const displayPrice = hasFlavours && currentFlavour ? currentFlavour.price * currentQty : item.price * currentQty;
                            const isCake = hasFlavours || item.category?.toLowerCase().includes('cake');

                            return (
                              <tr key={item._id} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)', transition: 'background 0.2s' }}>
                                <td style={{ padding: '1rem' }}>
                                  <div style={{ fontWeight: '600', color: 'var(--text-primary)', fontSize: '0.95rem' }}>{item.name}</div>
                                  {item.description && (
                                    <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: '0.2rem', fontWeight: '400', maxWidth: '350px', whiteSpace: 'normal' }}>
                                      {item.description}
                                    </div>
                                  )}
                                  {hasFlavours && (
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.5rem' }}>
                                      <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Flavour:</span>
                                      <select
                                        ...
                                        style={{ background: 'rgba(0,0,0,0.6)', border: '1px solid var(--border-gold)', color: 'var(--text-primary)', borderRadius: '6px', padding: '0.25rem 0.4rem', outline: 'none', fontSize: '0.75rem', cursor: 'pointer' }}
                                      >
                                        {item.flavours.map(f => (
                                  ...
                                  >
                                    {isCake ? (
                                      ...
                                      <>
                                        ...
                                        <option value="6" style={{ background: '#130a06' }}>6 Qty</option>
                                        <option value="10" style={{ background: '#130a06' }}>10 Qty</option>
                                      </>
                                    )}
                                  </select>
                                </td>
                                ...
                                <td style={{ padding: '1rem', textAlign: 'right' }}>
                                  <button
                                    onClick={() => openWhatsApp(item, currentFlavour, currentQty)}
                                    style={{
