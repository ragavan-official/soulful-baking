...
const Payment = ({ user }) => {
  ...
  const fetchCourseDetails = async () => {
    try {
      ...
      if (data.isPurchased) {
        // Already purchased, go straight to classroom
      ...
      const options = {
        ...
        handler: async function (response) {
          try {
            ...
            // 4. Trigger celebration and success screen
            confetti({
              particleCount: 150,
              spread: 80,
              origin: { y: 0.6 },
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '2.5rem', alignItems: 'start' }}>
        ...
        <div className="glass-card" style={{ padding: '2rem', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.95rem', marginBottom: '2rem', lineHeight: '1.6' }}>
            Click the button below to open our secure payment gateway. You can complete the transaction using cards, UPI apps, wallets, or net banking.
          </p>
